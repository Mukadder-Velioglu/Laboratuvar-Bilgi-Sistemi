...
if(isset($_POST['eposta']) and isset($_POST['sifre'])){
	...
	$kullanici=$database->get("kullanicilar","*",["AND"=>["eposta"=>$eposta,"sifre"=>$sifre, "gorev"=>["Sekreter"]]]);

	if($kullanici["id"]>0){
		$_SESSION["id"]=$kullanici["id"];
		header("Location:sekreterekran.php");
	}else{
		header("Location:sekreter.php?mesaj=hata2");
	}
}else{
	header("Location:sekreter.php?mesaj=hata1");
}
